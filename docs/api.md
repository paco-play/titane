...
Built-in components include `Transform`, `Mesh`, `Name`, `Velocity`, `Light`, `Gltf` (`url`, `clip`, `playing`, `loop`, `fade`), `Sound`, `Camera` (`fov`, `near`, `far`, `current`), `Skybox` (`color`, `cubemap` texture URL), `RigidBody` (`kind`, `friction`, `restitution`), `Collider` (`kind` box/sphere/capsule/mesh, `center`, `size`, `radius`, `height`), `Sensor`, `PlayerControlled`, `Input`. `pickCurrentCamera` / `setCurrentCamera` choose which camera Play and game mode use. `pickSkybox` selects the first authored sky; empty `cubemap` is a solid color (`#0a0a0a` when no component exists). A `Collider` overrides `Mesh.primitive` for physics. `mesh` colliders are Rapier trimeshes from the loaded glTF (not stored in `.titane`) and force a fixed body. Changing `Gltf.clip` while playing crossfades when `fade` is greater than 0; `fade === 0` is a hard cut.
