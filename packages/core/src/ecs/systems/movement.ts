 ...
 * units-per-second into a frame displacement. Entities with a `RigidBody` are
 * skipped — Rapier owns their motion.
 *
...
export const integrateVelocitySystem = (world: World, deltaTime: number): void => {
    for (const entityId of runQuery(world, movingQuery)) {
        if (hasComponent(world, entityId, RigidBody)) continue;
